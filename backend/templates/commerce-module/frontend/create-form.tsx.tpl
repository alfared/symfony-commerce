...
type Props = {
    onSubmit: (payload: Create{{ Entity }}Payload) => void;
};

export function Create{{ Entity }}Form({ onSubmit }: Props) {
    const [form, setForm] = useState<Create{{ Entity }}Payload>({
        code: '',
        name: '',
        slug: '',
        description: '',
    ...
    return (
        <form onSubmit={submit} className="grid gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-5">
            <h2 className="text-x1 font-semibold text-slate-50">Create {{ Entity }}</h2>
